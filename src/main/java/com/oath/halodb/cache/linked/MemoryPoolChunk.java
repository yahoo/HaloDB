...
import static com.oath.halodb.cache.linked.MemoryPoolHashEntries.*;

/**
 * Memory pool is divided into chunks of configurable size.
 * This represents such a chunk.
 *
...
class MemoryPoolChunk {
    ...
    /**
     * Relative put method.
     * Writes to the slot pointed to by the writeOffset and increments the writeOffset.
     */
    ...
    /**
     * Absolute put method.
     * Writes to the slot pointed to by the offset. 
     */
    void fillSlot(int slotOffset, byte[] key, byte[] value, MemoryPoolAddress nextAddress) {
        ...
        if (chunkSize - slotOffset < fixedSlotSize) {
            throw new IllegalArgumentException(String.format("Invalid offset %d. Chunk size %d. fixed slot size %d", slotOffset, chunkSize, fixedSlotSize));
        }
    ...
    void setValue(byte[] value, int slotOffset) {
        if (value.length != fixedValueLength) {
            throw new IllegalArgumentException(String.format("Invalid value length. fixedValueLength %d, value length %d", fixedValueLength, value.length));
        }
