...
abstract class Segment<V> {
    ...
    Segment(CacheSerializer<V> valueSerializer, int fixedValueLength, Hasher hasher) {
        this(valueSerializer, fixedValueLength, -1, hasher);
    }

    Segment(CacheSerializer<V> valueSerializer, int fixedValueLength, int fixedKeyLength, Hasher hasher) {
    ...
    void serializeForPut(byte[] key, V value, long hashEntryAdr) {
        try {
            Uns.keyBuffer(hashEntryAdr, key.length).put(key);
            if (value != null)
                valueSerializer.serialize(value, Uns.valueBuffer(hashEntryAdr, key.length, fixedValueLength));
        }
        catch (Throwable e) {
