...
class SegmentWithMemoryPool<V> extends Segment<V> {
    ...
    SegmentWithMemoryPool(OHCacheBuilder<V> builder) {
    ...
    static final class Table {
        ...
        static Table create(int hashTableSize) {
            int msz = Ints.checkedCast(Util.MEMORY_POOL_BUCKET_ENTRY_LEN * hashTableSize);
        ...
        void clear() {
            Uns.setMemory(address, 0L, Util.MEMORY_POOL_BUCKET_ENTRY_LEN * size(), (byte) -1);
        }
        ...
        long bucketOffset(long hash) {
            return bucketIndexForHash(hash) * Util.MEMORY_POOL_BUCKET_ENTRY_LEN;
        }
