...
public class MemoryPoolChunkTest {
    ...
    public void testSetAndGetMethods() {
        ...
        byte[] value = TestUtils.randomBytes(fixedValueLength);
        ...
        byte[] key2 = TestUtils.randomBytes(fixedKeyLength);
        byte[] value2 = TestUtils.randomBytes(fixedValueLength);
        ...
        byte[] value3 = TestUtils.randomBytes(fixedValueLength);
    ...
    public void testWithInvalidOffset() {
        ...
        chunk = MemoryPoolChunk.create(chunkSize, fixedKeyLength, fixedValueLength);
        chunk.fillSlot(chunkSize - 5, TestUtils.randomBytes(fixedKeyLength), TestUtils.randomBytes(fixedValueLength), next);
    }
    ...
    public void testWithInvalidKey() {
        ...
        MemoryPoolAddress next = new MemoryPoolAddress((byte)-1, -1);
        chunk = MemoryPoolChunk.create(chunkSize, fixedKeyLength, fixedValueLength);
        chunk.fillSlot(chunkSize - 5, TestUtils.randomBytes(fixedKeyLength+10), TestUtils.randomBytes(fixedValueLength), next);
    }
    ...
    public void testCompare() {
        ...
        chunk = MemoryPoolChunk.create(chunkSize, fixedKeyLength, fixedValueLength);
        byte[] key = TestUtils.randomBytes(fixedKeyLength);
        byte[] value = TestUtils.randomBytes(fixedValueLength);
        int offset = 0;
    ...
    public void testCompareKeyWithException() {
        ...
        byte[] bigKey = TestUtils.randomBytes(fixedKeyLength + 1);
    ...
    public void testCompareValueWithException() {
        ...
        byte[] bigValue = TestUtils.randomBytes(fixedValueLength + 1);
