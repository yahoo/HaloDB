...
public class SegmentWithMemoryPoolTest {
    ...
    public void testChunkAllocations() {
        ...
        OHCacheBuilder<byte[]> builder = OHCacheBuilder
            .<byte[]>newBuilder()
    ...
    public void testReplace() {
        ...
        for (int i = 0; i < noOfEntries; i++) {
            byte[] key = TestUtils.randomBytes(fixedKeySize);
        ...
        map.forEach((k, v) -> {
            Assert.assertTrue(segment.putEntry(k.buffer, TestUtils.randomBytes(fixedValueSize), k.hash(), false, v));
        });
